                        ...
                        <div class="card-body">
                            ...
                            <div class="pl-lg-4">
                                <form action="{{ route('submit.profile') }}" method="post">
                                    ...
                                    <input value="@if(!empty($userinfo->agent_id)){{$userinfo->agent_id}}@endif" name="agent_id" type="hidden">
                                    <div class="row">
                                        <div class="col-lg-6">
                                            <div class="form-group">
                                                <label class="form-control-label" for="phone">Phone Number</label>
                                                <input id="phone" name="phone" type="text" value="@if(!empty($userinfo->phone)){{ $userinfo->phone }}@endif" class="form-control">
                                            </div>
                                        </div>
                                        <div class="col-lg-6">
                                            <div class="form-group">
                                                <label class="form-control-label" for="gender">Gender</label>
                                                <input class="form-control" name="gender" value="@if(!empty($userinfo->gender)){{ $userinfo->gender }}@endif">
                                            </div>
                                        </div>
                                        <div class="col-lg-6">
                                            <div class="form-group">
                                                <label class="form-control-label" for="state">State</label>
                                                <input  name="state" type="text" value="@if(!empty($userinfo->state)){{ $userinfo->state }}@endif" class="form-control" disabled>
                                            </div>
                                        </div>
                                        <div class="col-lg-6">
                                            <div class="form-group">
                                                <label class="form-control-label" for="lga">LGA</label>
                                                <input class="form-control" name="lga" value="@if(!empty($userinfo->lga)){{ $userinfo->lga }}@endif" disabled>
                                            </div>
                                        </div>
                                        <button class="btn btn-success">Update</button>
                                    </div>
                                        ...
                                        <input type="hidden" name="id" value="{{ $user->id }}">
                                            <div class="form-group">
                                                ...
                                                <select name="role" class="form-control">
                                                    <option value="1" @if(in_array('superadmin', $user->roles()->get()->toArray())) selected @endif>Super Admin</option>
                            ...
                            <!-- Change Agent -->
                            <h6 class="heading-small text-muted mb-4">User Agent ({{ $assigned->name }})</h6>
                            <div class="pl-lg-4">
                                <form action="{{ route('submit.profile') }}" method="POST">
                                    @csrf
                                    <input type="hidden" name="id" value="{{ $user->id }}">
                                    <div class="form-group">
                                        <label class="form-control-label" for="agent">Select User Agent</label>
